...
export const scriptureChunkSchema = z
  .object({
    id: z.string().min(1),
  ...
  .transform((chunk) => ({
    ...chunk,
    verseStart: chunk.verseStart ?? chunk.verse,
    verseEnd: chunk.verseEnd ?? chunk.verseStart ?? chunk.verse,
  }))
  .superRefine((chunk, context) => {
    if (chunk.verseEnd < chunk.verseStart) {
      context.addIssue({
        code: "custom",
        path: ["verseEnd"],
